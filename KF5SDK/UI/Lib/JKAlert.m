...
+(id)showMessage:(NSString *)title message:(NSString *)message{
    ...
    [alert addButtonWithTitle:KF5Localized(@"kf5_confirm")];
    [alert show];
    return alert;
}
+(id)showMessage:(NSString *)message{
   return [[self class]showMessage:KF5Localized(@"kf5_reminder") message:message];
}

+ (id)showMessage:(NSString *)message OKHandler:(JKAlertHandler)handler{
    if (message == nil){
        return nil;
    }
    JKAlert *alert = [[JKAlert alloc]initWithTitle:KF5Localized(@"kf5_reminder") andMessage:message style:JKALERT_STYLE_ALERT];
    [alert addCancleButtonWithTitle:KF5Localized(@"kf5_cancel") handler:nil];
    [alert addCommonButtonWithTitle:KF5Localized(@"kf5_confirm") handler:handler];
    [alert show];
    return alert;
}
