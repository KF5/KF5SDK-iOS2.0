...
- (void)cancleRecording{
    ...
    if (self.isRecording) {
        ...
        if (self.receiveCancleBlock){
            self.receiveCancleBlock();
        }
    }
...
//    DLOG(@"录音发生错误");
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(recordFailure:)]){
        [self.delegate recordFailure:error];
    }
